...
const app = net.createServer(socket => {
  socket.on('data', data => {
    ...
    if(request.method === 'GET' && request.path === '/') {
      ...
      socket.end(createResponse({ body: colorHtml('Red'), status: '200 OK', contentType: 'text/html' }));
    } else if(request.method === 'GET' && request.path === '/green') {
      socket.end(createResponse({ body: colorHtml('Green'), status: '200 OK', contentType: 'text/html' }));
    } else if(request.method === 'GET' && request.path === '/blue') {
      socket.end(createResponse({ body: colorHtml('Blue'), status: '200 OK', contentType: 'text/html' }));
